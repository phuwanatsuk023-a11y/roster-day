          ...
          <nav className="flex items-center space-x-1 sm:space-x-2">
              ...
              <>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${
                    isSheetConnected
                      ? 'border-emerald-400 bg-emerald-800 text-white hover:bg-emerald-700'
                      : 'border-emerald-700 bg-emerald-950/60 text-emerald-200 hover:bg-emerald-800 hover:border-emerald-500'
                  }`}
                  title="ตั้งค่าเชื่อมต่อ Google Sheets API"
                >
                  <Database className={`w-3.5 h-3.5 ${isSheetConnected ? 'text-emerald-300' : 'text-emerald-400'}`} />
            ...
            {isAdmin ? (
              <div className="flex items-center gap-1.5 pl-2 border-l border-emerald-800">
                <span className="hidden xl:inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold rounded-lg bg-emerald-800 text-emerald-100 border border-emerald-700">
                  <Shield className="w-3 h-3 text-emerald-300" /> เจ้าหน้าที่ (Admin)
                </span>
                <button
                  id="btn-logout"
                  onClick={onLogout}
              ...
              <button
                ...
                onClick={onOpenLogin}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs sm:text-sm font-bold bg-white text-emerald-900 hover:bg-emerald-50 shadow-md transition-all"
              >
