...
export const PublicDutyCheck: React.FC<PublicDutyCheckProps> = ({
  ...
  const employeeDutyAssignments = useMemo(() => {
    ...
    // Group rows by day for inspector aggregation or normal duty
    // Structure:
    // {
    //   day: number;
    //   dateStr: string;
    ...
    scheduleRows.forEach(row => {
      ...
      const dayGroup = dayMap.get(row.day)!;

      const isInspector =
        row.inspector_id === selectedPersonId ||
        (Boolean(row.inspector_name) && row.inspector_name.includes(personFname));

      if (isInspector) {
        dayGroup.isInspector = true;
        dayGroup.inspectorRows.push(row);
      }

      let isNormalAssigned = false;
      let role = '';

      if (row.head_id === selectedPersonId || (row.head_name && row.head_name.includes(personFname))) {
        isNormalAssigned = true;
        role = 'หัวหน้าเวร';
      } else if (row.sub_id === selectedPersonId || (row.sub_name && row.sub_name.includes(personFname))) {
        isNormalAssigned = true;
        role = 'ผู้ช่วยเวร (ลำดับ 1)';
      } else if (row.sub2_id === selectedPersonId || (row.sub2_name && row.sub2_name.includes(personFname))) {
        isNormalAssigned = true;
      ...
      if (isNormalAssigned) {
        ...
        dayGroup.normalAssignments.push({
          day: row.day,
    ...
    dayMap.forEach((group, day) => {
      ...
      // Also add any normal assignment if assigned as head/sub (if not inspector or in addition)
      group.normalAssignments.forEach(na => {
        finalAssignments.push(na);
      });
    });

    return finalAssignments.sort((a, b) => a.day - b.day);
  }, [scheduleRows, selectedPersonId, selectedPerson, selectedMonth, selectedYear, currentDate]);
      ...
      <div className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-950 text-white rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden border border-emerald-700/40">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <div className="relative bg-white/10 backdrop-blur-md p-2 rounded-xl border border-white/20 shadow-inner flex flex-col sm:flex-row gap-2">
            ...
            {/* Month & Year Selectors with Light Green Background */}
            <div className="flex gap-2">
              <select
                ...
                className="bg-emerald-100 text-emerald-950 border border-emerald-300 hover:bg-emerald-200 text-xs sm:text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold shadow-sm transition-colors cursor-pointer"
              >
              ...
              >
                {[2025, 2026, 2027, 2028].map(y => (
                  <option key={y} value={y} className="bg-white text-slate-900 font-medium">
                    {y + 543}
                  </option>
                ))}
              </select>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-2.5 text-xs text-slate-600 dark:text-slate-300">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="bg-slate-50 dark:bg-slate-700/40 p-2.5 rounded-xl">
                              <span className="text-slate-400 block text-[10px] flex items-center gap-1">
                                <Shield className="w-3 h-3 text-emerald-600" /> บทบาทในเวร:
                              </span>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-700/40 p-2.5 rounded-xl">
                              <span className="text-slate-400 block text-[10px] flex items-center gap-1">
                                <Clock className="w-3 h-3 text-emerald-600" /> เวลาปฏิบัติงาน:
                              </span>
                          ...
                          {/* IF INSPECTOR: Show all duty points and personnel under inspection aggregated in one card */}
                          {assignment.isInspector && assignment.inspectorPoints && assignment.inspectorPoints.length > 0 ? (
                            <div className="space-y-2">
                              <div className="flex items-center justify-between text-xs font-bold text-emerald-900 dark:text-emerald-200 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-1.5 rounded-lg border border-emerald-200 dark:border-emerald-800">
                                <span className="flex items-center gap-1">
                                  <MapPin className="w-3.5 h-3.5 text-emerald-600" />
                                  <span>รายชื่อผู้อยู่เวรแต่ละจุดที่ต้องตรวจ ({assignment.inspectorPoints.length} จุด):</span>
                                </span>
                              </div>

                              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                                {assignment.inspectorPoints.map((pt, pIdx) => (
                                  <div
                                    key={pIdx}
                                    className="bg-slate-50 dark:bg-slate-700/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700 space-y-1"
                                  >
                                    <div className="flex items-center justify-between font-bold text-slate-900 dark:text-white text-xs border-b border-slate-200/60 dark:border-slate-600 pb-1">
                                      <span className="text-emerald-800 dark:text-emerald-300 flex items-center gap-1">
                                        <MapPin className="w-3 h-3 text-emerald-600" />
                                        {pt.pointName}
                                      </span>
                                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold ${
                                        pt.gender === 'M' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200' : 'bg-teal-100 text-teal-800 dark:bg-teal-900/60 dark:text-teal-200'
                                      }`}>
                                        {pt.gender === 'M' ? 'เวรกลางคืน' : 'เวรกลางวัน'}
                                      </span>
                                    </div>

                                    <div className="space-y-0.5 text-[11px] pt-0.5 text-slate-700 dark:text-slate-300">
                                      {pt.headName && (
                                        <div className="flex items-center justify-between">
                                          <span className="font-semibold text-slate-900 dark:text-white">1. {pt.headName}</span>
                                          <span className="text-[10px] text-emerald-700 dark:text-emerald-400 font-bold">(หน.เวร)</span>
                                        </div>
                                      )}
                                      {pt.subName && (
                                        <div className="flex items-center justify-between">
                                          <span>2. {pt.subName}</span>
                                          <span className="text-[10px] text-slate-500">(ผช.เวร)</span>
                                        </div>
                                      )}
                                      {pt.sub2Name && (
                                        <div className="flex items-center justify-between">
                                          <span>3. {pt.sub2Name}</span>
                                          <span className="text-[10px] text-slate-500">(ผช.เวร)</span>
                                        </div>
                                      )}
                                      {!pt.headName && !pt.subName && !pt.sub2Name && (
                                        <span className="text-slate-400 italic text-[10px]">ไม่มีรายชื่อผู้เข้าเวร</span>
                                      )}
                                    </div>
                                  </div>
                                ))}
